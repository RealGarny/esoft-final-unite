...
const CommunityCard = () => {
    return(
        <Card padding="none" className="relative flex-col gap-0 flex-grow-0 flex-shrink-0 w-96">
            <Link to={`${routes.communities}/${requiredProps.CID}`} className="absolute z-30 top-0 left-0 w-full h-full"/>
            ...
            <Flexbox className="flex-col relative px-5 py-5 gap-4 justify-between">
                <Card rounded="xl" padding="none" className=" h-24 w-24 mt-[-70px] bottom-2 border-black border-4 border-opacity-10">
                    <img src={requiredProps.iconURL}></img>
                </Card>
                ...
                <Flexbox padding="none" className="flex-col font-bold w-full text-sm h-36 overflow-hidden">
                    <Card padding="none" rounded="sm" className="px-1 uppercase bg-accent text-sm font-bold min-h-6 w-fit ">New!</Card>

...
interface FormElement extends InputHTMLAttributes<HTMLTextAreaElement>, VariantProps<typeof TextareaVariants> {
}
...
const Input:React.FC<FormElement> = ({variant, padding, rounded, className, ...args}) => {
    return(
        <textarea
            {...args}
            className={cn(TextareaVariants({variant, padding, rounded, className}))}
        />
    )
}
